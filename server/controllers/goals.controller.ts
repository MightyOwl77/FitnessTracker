...
/**
 * Goals controller with proper validation and error handling
 */
export const goalsController = {
  /**
   * Get user goals
   */
  getUserGoals: async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Unauthorized" });
   ...
   * Create or update user goals
   */
  createOrUpdateGoals: async (req: AuthRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Unauthorized" });
