...
const goalsSchema = z.object({
  ...
  // Although the deficit slider UI is removed, the default deficit rate remains for calculation.
    ...
    for (let i = 0; i <= estWeeks; i++) {
      projectionData.push({
        ...
        weight: i === estWeeks
          ...
          : Math.max(targetWeight, currentWeight - projectedWeeklyLossRate * i).toFixed(1)
      });
    }
    ...
    <div className="py-6">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onNext)} className="space-y-6">
          ...
          />
          {/* Additional fields (gender, height, weight, etc.) should be added similarly */}
...
function DeficitPlanStep({
  ...
  // Daily Summary: Total deficit = Base TDEE (Calories Out) - Adjusted Calorie Target (Calories In)
  return (
    <div className="py-6">
      ...
      <div className="bg-secondary/30 p-6 rounded-lg mb-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Adjust your daily calorie intake below. The slider minimum is set to 25% below your base maintenance (TDEE).
        </p>
        <div className="bg-background p-4 rounded-lg mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="bg-primary/5 p-3 rounded-lg">
              <div className="text-sm text-muted-foreground">Maintenance Calories</div>
              <div className="text-2xl font-bold">{baseTDEE.toLocaleString()} cal</div>
              <div className="text-xs text-muted-foreground">Calculated TDEE</div>
            </div>
            <div className="bg-primary/5 p-3 rounded-lg">
              <div className="text-sm text-muted-foreground">Daily Calorie Target</div>
              <div className="text-2xl font-bold">{adjustedCalorieTarget.toLocaleString()} cal</div>
              <div className="text-xs text-muted-foreground">
                {adjustedCalorieTarget < baseTDEE
                  ? `${baseTDEE - adjustedCalorieTarget} calories deficit`
                  : "At maintenance"}
              </div>
            </div>
          </div>
          <div className="mb-8">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className="font-medium text-green-600">
                {adjustedCalorieTarget < baseTDEE ? `-${baseTDEE - adjustedCalorieTarget} calories` : "No deficit"}
              </span>
            </div>
            ...
            <div className="flex justify-between text-xs mt-1">
              <span className="text-muted-foreground">{Math.round(baseTDEE * 0.75).toLocaleString()} cal (min intake)</span>
              <span className="text-muted-foreground">{baseTDEE.toLocaleString()} cal (maintenance)</span>
            </div>
          </div>
          {/* Daily Summary Block */}
          <div className="bg-background p-4 rounded-lg mb-4 border">
            <h4 className="text-md font-medium mb-2">Daily Summary</h4>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <p className="text-sm text-muted-foreground">Calories Out</p>
                <p className="text-xl font-bold">{baseTDEE.toLocaleString()} cal</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Calories In</p>
                <p className="text-xl font-bold">{adjustedCalorieTarget.toLocaleString()} cal</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Daily Deficit</p>
                <p className="text-xl font-bold">{(baseTDEE - adjustedCalorieTarget).toLocaleString()} cal</p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground mt-2">
              Total deficit = Calories Out (maintenance) minus Calories In (target).
            </p>
          </div>
          ...
          <div className="bg-primary/5 p-3 rounded-lg">
            ...
            <div className="text-lg font-bold text-green-800">
              {adjustedCalorieTarget < baseTDEE ? `${baseTDEE - adjustedCalorieTarget} calories deficit` : 'No calorie deficit'}
            </div>
            <div className="text-xs text-muted-foreground">
              {adjustedCalorieTarget < baseTDEE 
                ? `Weekly fat loss potential: ${((baseTDEE - adjustedCalorieTarget) * 7 / 7700).toFixed(2)} kg` 
...
export default function Onboarding() {
  ...
  // Update profile and preferences forms when data arrives
  useEffect(() => {
    if (profileData) {
      const formValues = {
    ...
    try {
      ...
      if (adjustedCalorieTarget === 2000) {
        const newTarget = Math.round(
          tdee -
          Math.max(0, Math.round(((goals.deficitRate || 0.5) / 100 * currentWeight) * 7700 / 7) - dailyActivityCalories)
        );
  ...
  // Warn if leaving mid-onboarding
  useEffect(() => {
  ...
  const { height, age, gender, activityLevel } = profileForm.watch();
  useEffect(() => {
    try {
      const baseBMR = calculateBMR(currentWeight, height || 175, age || 30, gender || "male");
      const calculatedTDEE = Math.round(calculateTDEE(baseBMR, activityLevel || "moderately"));
